...
l<- file.choose()
n<- file.choose()
link.data<- read.csv(l, header = TRUE, stringsAsFactors = FALSE)
node.data<- read.csv(n, header = TRUE, stringsAsFactors = FALSE)
...
barplot(sort(V(g)$deg.out), main = "Most Friendly" ,horiz=TRUE, las=2,names.arg = V(g)$name)
barplot(sort(V(g)$deg.in), main = "Most Popular" ,horiz=TRUE, las=2,names.arg = V(g)$name)
...
my.dir<-"C:\\Users\\laura\\OneDrive\\Documents\\Syracuse iSchool\\IST 719\\IST-719\\Labs\\Lab 8\\"
